// Stripping Names: Store a person’s name, and include some whitespace
// characters at the beginning and end of the name. Make sure you use each
// character combination, "\t" and "\n", at least once. Print the name
// once, so the whitespace around the name is displayed. Then print the
// name after striping the white spaces.

let namewihtwhitespace = ("\t\n Waleed \t\n");
console.log("Orignal Name:", namewihtwhitespace);

let StrippingName = (namewihtwhitespace);
console.log("Stripping Name:", StrippingName);
